...
class FixedGame:
    """Fixed game class that uses ProperBot"""
    ...
    def play(self, max_turns: int = 200) -> Tuple[Optional[str], List[str]]:
        """
        Play a game between two bots
        
        Args:
            max_turns: Maximum number of turns (safety limit to prevent infinite games)
        ...
        try:
            ...
            if move1 == "-1 -1 -1 -1 -1 -1":
                ...
                print(f"  {self.bot1_name} has no legal moves")
            ...
            while True:
                ...
                if turn > max_turns:
                    print(f"  Warning: Game exceeded maximum turns ({max_turns})")
                    self.winner = "Draw (max turns reached)"
                    break
        ...
        if self.error:
            ...
            if self.winner == "Draw (max turns reached)":
                print(f"  Game finished: {self.winner} after {len(self.moves)} moves")
            else:
...
def test_bot000_vs_bot003():
    ...
    winner, moves = game.play(max_turns=50)  # Increased to 50 moves
    
    if game.error:
        print(f"✗ Test failed: Error in bot000 vs bot003: {game.error}")
        return False
    elif winner == "Draw (max turns reached)":
        print(f"✓ Test passed: bot000 vs bot003 played {len(moves)} moves without errors")
        print(f"  Game ended at max turns - both bots working correctly")
        return True
    else:
