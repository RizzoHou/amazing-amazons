...
double calculate_move_priority(const Move& m) {
    // Center proximity: moves closer to board center are better
    // Board center is at (3.5, 3.5), we use Manhattan distance from center
    double center_x = 3.5;
    double center_y = 3.5;
    
    // Distance of piece destination from center (lower is better)
    double dest_dist = abs(m.x1 - center_x) + abs(m.y1 - center_y);
    // Distance of arrow from center (lower is better)
    double arrow_dist = abs(m.x2 - center_x) + abs(m.y2 - center_y);
    
    // Mobility impact: moves that shoot arrows closer to opponent pieces are better
    // Simple heuristic: arrow distance from center (closer to center is better)
    // Combine with piece movement distance (shorter moves may be better)
    double move_length = abs(m.x1 - m.x0) + abs(m.y1 - m.y0);
    
    // Priority formula: higher priority is better
    // We want to minimize dest_dist, arrow_dist, and move_length
    double priority = 100.0 - (dest_dist + arrow_dist * 0.5 + move_length * 0.2);
    
    // Bonus for moves that stay in the center region (3,3)-(4,4)
    if (m.x1 >= 3 && m.x1 <= 4 && m.y1 >= 3 && m.y1 <= 4) priority += 20.0;
    if (m.x2 >= 3 && m.x2 <= 4 && m.y2 >= 3 && m.y2 <= 4) priority += 10.0;
...
void order_moves(vector<Move>& moves) {
    sort(moves.begin(), moves.end(), [](const Move& a, const Move& b) {
        return calculate_move_priority(a) > calculate_move_priority(b);
    });
}
...
const int ZOBRIST_SIZE = GRID_SIZE * GRID_SIZE * 4; // 8x8 board, 4 possible states (EMPTY, BLACK, WHITE, OBSTACLE)
uint64_t zobrist_table[ZOBRIST_SIZE];

// Initialize Zobrist table with random 64-bit numbers
void init_zobrist() {
    static bool initialized = false;
    if (initialized) return;
    
    mt19937_64 rng(123456789); // Fixed seed for reproducibility
...
uint64_t zobrist_hash(const Board& board) {
    ...
    for (int x = 0; x < GRID_SIZE; x++) {
        for (int y = 0; y < GRID_SIZE; y++) {
            ...
            if (piece != EMPTY) {
                // Map piece to index: EMPTY=0, BLACK=1, WHITE=2, OBSTACLE=3
                int piece_index;
                if (piece == BLACK) piece_index = 1;
                else if (piece == WHITE) piece_index = 2;
                else if (piece == OBSTACLE) piece_index = 3;
                else piece_index = 0;
                
                int table_index = (x * GRID_SIZE + y) * 4 + piece_index;
                hash ^= zobrist_table[table_index];
            }
        }
    }
    return hash;
...
struct TTEntry {
    uint64_t hash;
    double eval;
    int depth; // Depth of search when cached (not used in simple version)
    bool valid;
    
    TTEntry() : hash(0), eval(0.0), depth(0), valid(false) {}
};

// Transposition table with LRU cache (simplified fixed-size array)
class TranspositionTable {
private:
    static const int TABLE_SIZE = 1 << 16; // 65536 entries
    TTEntry table[TABLE_SIZE];
    int accesses;
    
public:
    TranspositionTable() : accesses(0) {
    ...
    // Lookup evaluation for a hash
    bool lookup(uint64_t hash, double& eval) {
        int index = hash % TABLE_SIZE;
        if (table[index].valid && table[index].hash == hash) {
    ...
    // Store evaluation for a hash
    void store(uint64_t hash, double eval) {
        int index = hash % TABLE_SIZE;
        table[index].hash = hash;
        table[index].eval = eval;
        table[index].valid = true;
        // Simple LRU: if we've accessed many times, occasionally clear old entries
        accesses++;
        if (accesses % 10000 == 0) {
            // Clear 10% of entries
...
public:
    ...
    int expanded_count; // Number of moves expanded so far (for progressive widening)
    
    MCTSNode(MCTSNode* p = nullptr, Move m = Move(), int pjm = 0)
        : parent(p), move(m), wins(0.0), visits(0), player_just_moved(pjm), expanded_count(0) {}
...
public:
    ...
    double evaluate_multi_component(const array<array<int, GRID_SIZE>, GRID_SIZE>& grid, 
                                    int root_player) {
        // Create a temporary board to compute hash
        Board temp_board;
        temp_board.grid = grid;
        uint64_t hash = zobrist_hash(temp_board);
        
        // Try to lookup in transposition table
        double cached_eval;
        if (tt.lookup(hash, cached_eval)) {
            return cached_eval;
        }
    ...
    Move search(const Board& root_state, int root_player) {
        if (root == nullptr) {
            ...
            // Apply move ordering to untried moves
            order_moves(root->untried_moves);
        }
        ...
        while (true) {
            ...
            if (!node->untried_moves.empty()) {
                // Determine how many moves we should have expanded based on visits
                // Progressive widening: start with top 5 moves, gradually expand more
                int target_expanded = 5; // Base number
                if (node->visits > 50) target_expanded = 10;
                if (node->visits > 200) target_expanded = 20;
                if (node->visits > 500) target_expanded = 30;
                // Cap at total number of untried moves
                if (target_expanded > node->untried_moves.size()) {
                    target_expanded = node->untried_moves.size();
                }
                
                // If we haven't expanded enough moves yet, expand one more
                if (node->expanded_count < target_expanded) {
                    // Pick the next move to expand (based on ordering)
                    Move m = node->untried_moves[node->expanded_count];
                    ...
                    // Order the new node's untried moves
                    order_moves(new_node->untried_moves);
                    
                    // Move from untried_moves to children
                    // We don't erase from untried_moves because we're using expanded_count as index
                    node->children.push_back(new_node);
                    node->expanded_count++;
                    node = new_node;
                } else {
                    // All allowed moves are expanded, need to select among children
                    // This will be handled by the selection phase in next iteration
                    // For now, we'll fall back to selection
                    if (!node->children.empty()) {
                        node = node->uct_select_child(C);
                        state.apply_move(node->move);
                        current_player = -current_player;
                    } else {
                        // No children expanded yet (should not happen if target_expanded > 0)
                        // Expand the first move anyway
                        Move m = node->untried_moves[0];
                        state.apply_move(m);
                        current_player = -current_player;
