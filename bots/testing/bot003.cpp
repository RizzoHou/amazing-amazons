...
public:
    ...
    Move search(const Board& root_state, int root_player, int& iterations) {
        if (root == nullptr) {
        ...
        // cout << "[DEBUG] " << iterations << " iterations" << endl;
...
int main() {
    ...
    int iterations;
    Move best_move = ai.search(board, my_color, iterations);
    ...
    while (true) {
        try {
            ...
            best_move = ai.search(board, my_color, iterations);
