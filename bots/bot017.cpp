...
public:
    ...
    // Constructor with optional initialization control
    Board(bool do_init) {
        for (int i = 0; i < GRID_SIZE; i++) {
            for (int j = 0; j < GRID_SIZE; j++) {
                grid[i][j] = EMPTY;
    ...
    // Optimized copy that skips init_board()
    Board copy() const {
        Board new_board(false);  // Skip init_board
...
public:
    ...
    int turn_number;
    mt19937 rng;
    MCTSNode* best_child;  // Track best child incrementally
    int max_visits;        // Current maximum visits
    
    MCTS(double tl = 5.0) : time_limit(tl), root(nullptr), turn_number(0), best_child(nullptr), max_visits(-1) {
    ...
    Move search(const Board& root_state, int root_player, const chrono::steady_clock::time_point& program_start_time, double original_time_limit, double safety_margin) {
        ...
        while (true) {
            // Check time only every 256 iterations (0xFF mask)
            if ((iterations & 0xFF) == 0) {
                if (chrono::steady_clock::now() >= deadline) break;
            }
            
            MCTSNode* node = root;
            Board state = root_state;  // Use implicit copy constructor instead of copy()
            ...
            // Expansion with swap-pop optimization
            if (!node->untried_moves.empty()) {
                // Use simple modulo for random selection (faster than uniform_int_distribution)
                ...
                // Swap-pop instead of erase (O(1) instead of O(n))
            ...
            // Backpropagation with incremental best child tracking
            while (node != nullptr) {
                node->visits++;
                
                // Update incremental best child tracking for root's children
        ...
        // Return the incrementally tracked best child
        if (best_child) {
            return best_child->move;
        } else {
            // Fallback: return first child (shouldn't happen if we had iterations)
...
const double SAFETY_MARGIN = 0.20;  // Safety buffer to ensure we finish before timeout
...
int main() {
    ...
    // Output the move with casting to int for int8_t coordinates
    if (best_move.x0 != -1) {
