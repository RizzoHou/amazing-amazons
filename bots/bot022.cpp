...
const int MAX_MOVES_POOL = 80000000; // 33000000
...
void run_bfs(const int8_t* grid, const vector<int>& sources, int* dist_out) {
    // Fill with 99
...
inline int calc_mobility(const int8_t* grid, const vector<int>& pieces) {
    int mob = 0;
    for (int p : pieces) {
        int px = p / 8;
        int py = p % 8;
...
double evaluate(const Board& board, int root_player, int turn) {
    vector<int> my_pieces, opp_pieces;
    my_pieces.reserve(4); opp_pieces.reserve(4);
    
    for(int i=0; i<NUM_SQUARES; ++i) {
        if(board.grid[i] == root_player) my_pieces.push_back(i);
        else if(board.grid[i] == -root_player) opp_pieces.push_back(i);
    }
