// bot013.cpp
// Based on bot012.cpp with memory optimizations from docs/references/gpt/re.md
//
// MEMORY FIXES APPLIED (to prevent SIGKILL/RE on Botzone):
// 1. Reduced reserve size from 4096 to 256 (prevents memory explosion per node)
// 2. Shrunk Move struct using int8_t (6 bytes instead of 24 bytes)
// 3. Made MCTSNode destructor non-recursive (~MCTSNode() = default)
// 4. Added incremental garbage collection (gc_stack + gc_collect_some)
// 5. Updated advance_root() to detach and enqueue old trees for GC
// 6. Cap untried_moves at deep nodes to prevent worst-case bloat
//
// INHERITED FROM bot012:
// - Deadline-based timing with safety margin
// - Board::fast_copy() method
// - Strategic time_up() checks
// - Modulo RNG
...
// FIX 3A: Shrunk Move struct using int8_t (6 bytes instead of 24 bytes)
// On 8x8 board, coords fit in 0..7 (and -1 sentinel fits in int8_t)
struct Move {
...
public:
    ...
    vector<Move> get_legal_moves(int color) const {
        vector<Move> moves;
        moves.reserve(256); // FIX 1: Reduced from 4096 to prevent memory explosion per node
...
public:
    ...
    // FIX 2A: Non-recursive destructor - children managed by incremental GC
    ~MCTSNode() = default;
...
public:
    ...
    int turn_number;
    mt19937 rng;
    vector<MCTSNode*> gc_stack;  // FIX 2B: Garbage collection stack for incremental deletion
    ...
    ~MCTS() {
        // Clean up any remaining nodes in gc_stack
    ...
    // FIX 2B: Incremental garbage collection - deletes nodes without blocking
    ...
    Move search(const Board& root_state, int root_player) {
        ...
        // FIX 2D: Run incremental garbage collection at start of search
        // This cleans up old tree nodes without blocking (amortized deletion)
        ...
        while (!time_up()) {
            ...
            if (!node->untried_moves.empty()) {
                ...
                // FIX 3B: Cap untried_moves at deep nodes to prevent worst-case memory bloat
                // This trades some search completeness for memory stability
                auto& um = new_node->untried_moves;
                const int CAP = 128;
                if ((int)um.size() > CAP) {
                    // Random partial shuffle then truncate - ensures variety in move selection
                    for (int i = 0; i < CAP; i++) {
    ...
    // FIX 2C: Updated advance_root to detach and enqueue old trees for incremental GC
    // This prevents both memory leaks AND deletion spikes
    void advance_root(const Move& move) {
        ...
        if (new_root != nullptr) {
            ...
            // Old root now contains only "discarded" siblings; enqueue old root for GC
            gc_stack.push_back(root);
...
int main() {
    ...
    // FIX 3A: Cast int8_t to int when printing to avoid character output
    if (best_move.x0 != -1) {
    ...
    while (true) {
        try {
            ...
            // FIX 3A: Cast int8_t to int when printing
            if (best_move.x0 != -1) {
                cout << int(best_move.x0) << " " << int(best_move.y0) << " " 
                     << int(best_move.x1) << " " << int(best_move.y1) << " "
                     << int(best_move.x2) << " " << int(best_move.y2) << endl;
