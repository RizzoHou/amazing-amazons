// bot011.cpp
// Based on bot010.cpp with TLE optimizations from docs/references/gemini/tle.md
//
// OPTIMIZATIONS APPLIED:
// 1. Swap-and-pop for move removal (O(1) instead of O(N))
// 2. Static vectors in evaluate_optimized to prevent heap allocations
// 3. Pre-allocated move vector with reserve(128)
// 4. Increased time safety margins (0.72s instead of 0.88s)
...
public:
    ...
    vector<Move> get_legal_moves(int color) const {
        vector<Move> moves;
        moves.reserve(128); // OPTIMIZATION 3: Pre-allocate to prevent multiple reallocations
    ...
    Move search(const Board& root_state, int root_player) {
        ...
        while (true) {
            ...
            if (!node->untried_moves.empty()) {
                ...
                // OPTIMIZATION 1: Swap-and-pop instead of erase (O(1) instead of O(N))
...
// OPTIMIZATION 4: Increased time safety margins (280ms buffer instead of 120ms)
const double TIME_LIMIT = 0.72;
